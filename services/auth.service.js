...
const authService = {
  findUserByEmail: async (email) => {
    const result = await db.query(
      'SELECT id, email, password_hash, full_name FROM users WHERE email = $1 AND status = $2',
      [email, 'active']
    );
  ...
  validateUserContext: async (userId, tenantId, instituteId, roleId) => {
    const query = `
      ...
      WHERE user_id = $1 AND tenant_id = $2 AND institute_id = $3 AND role_id = $4 AND status = $5
    `;
    const result = await db.query(query, [userId, tenantId, instituteId, roleId, 'active']);
